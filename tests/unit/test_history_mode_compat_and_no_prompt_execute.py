...
def test_history_mode_compat(monkeypatch, capsys):
    ...
    entries = [
        {"timestamp": "1", "service": "s3", "safety_level": "SAFE", "command": "aws s3 ls", "query": "list s3", "execution_allowed": False, "execution_mode": "manual"}
    ]
...
def test_execute_no_prompt_blocks(monkeypatch, capsys):
    ...
    fake_result = {"command": "aws s3 rb s3://bucket --force", "safety": {"level": "DESTRUCTIVE"}, "intent": "delete"}
    monkeypatch.setattr(aws_nlp, "generate_command_sync", lambda q: fake_result)
    monkeypatch.setattr(sys, "argv", ["aws-nlp", "--execute", "--no-prompt", "delete s3 bucket x"])
    try:
        aws_nlp.main()
