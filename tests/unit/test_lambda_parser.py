...
import importlib
lambda_parser = importlib.import_module("parsers.lambda.parser")

def test_list_lambda_functions():
    result = lambda_parser.generate_command("list_lambda_functions", {"region": "us-west-1"})
    assert "list-functions" in result["command"]
    assert "us-west-1" in result["command"]

def test_invoke_lambda_function():
    result = lambda_parser.generate_command("invoke_lambda_function", {"function": "test-func", "region": "us-east-1"})
