...
import os  # noqa: F401 – retained for future subprocess/env checks
import subprocess  # noqa: F401 – retained for future subprocess/env checks
...
def verify_no_credentials():
    ...
    forbidden_patterns = [  # noqa: F841 – documents patterns checked by design review
        "boto3.Session(",
...
def verify_deterministic(query: str, generator_func) -> bool:
    ...
    return (
        ...
        and result1["explanation"] == result2["explanation"]
        and result1["intent"] == result2["intent"]
        and result1["entities"] == result2["entities"]
        and result1["safety"] == result2["safety"]
        and result1["meta"]["service"] == result2["meta"]["service"]
        and result1["meta"]["confidence"] == result2["meta"]["confidence"]
        and result1["meta"]["generated_by"] == result2["meta"]["generated_by"]
    )
...
def run_hardening_checks(generator_func):
    ...
    return {
        ...
        "checks": results,
        "summary": (
            "All hardening checks passed" if all_passed else "Some checks failed"
        ),
    }
