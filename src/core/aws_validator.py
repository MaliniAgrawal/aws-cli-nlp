...
import boto3  # noqa: F401 – optional; used only when ENABLE_AWS_VALIDATION=true
...
def validate_command_safe(intent: str, entities: dict) -> dict:
    ...
    if safety["level"] == "DESTRUCTIVE":
        return {
            "status": "dangerous",
            "reason": "Performs destructive operations",
    ...
    if safety["level"] == "SECURITY_SENSITIVE":
        return {
            "status": "warning",
            "reason": "Modifies security settings",
            "requires_confirmation": True,
    ...
    if safety["level"] == "MUTATING":
        return {
            "status": "warning",
            "reason": "Creates or modifies resources",
            "requires_confirmation": True,
...
def check_s3_bucket_exists(bucket_name, aws_session=None):
    ...
    try:
        ...
        return {
            "status": "invalid",
            "reason": f"Bucket '{bucket_name}' already exists.",
        }
