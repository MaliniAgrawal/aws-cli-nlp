...
RE_BUCKET = re.compile(
    r"\b(?:s3\s+bucket|bucket)\s+(?:(?:named|called|name)\s+)?([a-z0-9][a-z0-9\-\._]{2,62})\b",
    re.I,
)
RE_REGION = re.compile(r"\b(us|eu|ap|sa|ca|me)[\-\_]?[a-z0-9\-]+-\d\b", re.I)
RE_NAME = re.compile(r"\bname[d]?[:\s]+([A-Za-z0-9\-\_]+)\b", re.I)
RE_TABLE = re.compile(
    r"\b(?:table|dynamodb table)\s+(?:named|called)?\s*([A-Za-z0-9\-\_]+)\b", re.I
)
RE_DB = re.compile(
    r"\b(?:instance|database|db)\s+(?:named|called)?\s*([A-Za-z0-9\-\_]+)\b", re.I
)
RE_POLICY_ARN = re.compile(r"(arn:aws:iam::aws:policy/[A-Za-z0-9+=,.@_-]+)", re.I)
RE_POLICY_NAME = re.compile(
    r"(ReadOnlyAccess|AdministratorAccess|PowerUserAccess)", re.I
)
RE_USER = re.compile(
    r"\buser\s+(?:(?:named|called|name)\s+)?([A-Za-z0-9\-\_]+)\b", re.I
)
RE_FUNCTION = re.compile(
    r"\bfunction\s+(?:(?:named|called|name)\s+)?([A-Za-z0-9\-\_]+)\b", re.I
)
RE_QUEUE = re.compile(
    r"\bqueue\s+(?:(?:named|called|name)\s+)?([A-Za-z0-9\-\_]+)\b", re.I
)
RE_TOPIC = re.compile(
    r"\btopic\s+(?:(?:named|called|name)\s+)?([A-Za-z0-9\-\_]+)\b", re.I
)
RE_STACK = re.compile(
    r"\bstack\s+(?:(?:named|called|name)\s+)?([A-Za-z0-9\-\_]+)\b", re.I
)
...
S3_RULES = [
    ...
    (
        ...
        (
            "create_s3_bucket",
            lambda m: {"bucket": m.group(1), "region": (m.group(2) or None)},
        ),
    ),
    # list buckets
    (r"(?:list|show)\s+(?:all\s+)?s3\s+buckets", ("list_s3_buckets", lambda m: {})),
    # delete bucket
    (
    ...
    (
        ...
        (
            ...
            lambda m: {
                "bucket": m.group(1),
                "prefix": m.group(2) if m.group(2) else None,
            },
        ),
    ),
    # put object (upload)
    (
        ...
        (
            ...
            lambda m: {
                ...
                "bucket": m.group(2),
                "key": m.group(3) if m.group(3) else None,
            },
        ),
    ),
]
...
IAM_INTENT_PATTERNS = [
    (
        r"(?:create|make|add)\s+(?:iam\s+)?user(?:\s+(?:named|called|name))?\s+([A-Za-z0-9\-\_]+)",
        ("create_iam_user", lambda m: {"user": m.group(1)}),
    ),
    (
        r"(?:delete|remove)\s+(?:production\s+)?(?:iam\s+)?user(?:\s+(?:named|called|name))?\s+([A-Za-z0-9\-\_]+)",
        ("delete_iam_user", lambda m: {"user": m.group(1)}),
    ),
    (
        r"attach\s+(\w+)\s+to\s+user\s+(\w+)",
        ("attach_user_policy", lambda m: {"policy": m.group(1), "user": m.group(2)}),
    ),
    (
        r"attach\s+.*policy.*\s+to\s+user\s+(\w+)",
        ("attach_user_policy", _extract_iam_entities),
    ),
    (
        r"detach\s+(\w+)\s+from\s+user\s+(\w+)",
        ("detach_user_policy", lambda m: {"policy": m.group(1), "user": m.group(2)}),
    ),
    (
        r"detach\s+.*policy.*\s+from\s+user\s+(\w+)",
        ("detach_user_policy", _extract_iam_entities),
    ),
    (
        r"list\s+.*policies.*\s+user\s+(\w+)",
        ("list_attached_user_policies", lambda m: {"user": m.group(1)}),
    ),
]
...
RULES = S3_RULES + [
    ...
    for pattern, (intent, extractor) in IAM_INTENT_PATTERNS
]
...
def _get_local_classifier():
    ...
    try:
        _CLASSIFIER = pipeline(
            "zero-shot-classification", model="typeform/distilbert-mnli"
        )  # smaller model id
...
def parse_nlp(text: str):
    ...
    if ("dynamodb" in low and "create" in low) or ("create" in low and "dynamo" in low):
        return "create_dynamodb_table", {
            "table": table or name or "myTable",
            "region": region,
        }
    ...
    if "lambda" in low and "invoke" in low:
        return "invoke_lambda_function", {
            "function": function or name or "my-function",
            "region": region,
        }
    ...
    if ("rds" in low and ("delete" in low or "remove" in low)) or (
        "database" in low and ("delete" in low or "remove" in low)
    ):
        if db or name:
    ...
    if "sqs" in low and "create" in low:
        return "create_sqs_queue", {
            "queue": queue or name or "my-queue",
            "region": region,
        }
    ...
    if "sns" in low and "create" in low:
        return "create_sns_topic", {
            "topic": topic or name or "my-topic",
            "region": region,
        }
    if "sns" in low and "list" in low:
        return "list_sns_topics", {"region": region} if region else {}
    if ("cloudformation" in low and "create" in low) or (
        "stack" in low and "create" in low
    ):
        return "create_cloudformation_stack", {
            "stack": stack or name or "mystack",
            "region": region,
        }
    if ("cloudformation" in low and "list" in low) or (
        "stack" in low and "list" in low
    ):
