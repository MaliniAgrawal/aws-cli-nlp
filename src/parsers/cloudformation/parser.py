...
def generate_command(intent, entities):
    if intent == "create_cloudformation_stack":
        ...
        return {"command": cmd, "explanation": f"Creates CloudFormation stack '{stack}' using local template.yaml."}
    if intent == "list_cloudformation_stacks":
        return {"command":"aws cloudformation list-stacks","explanation":"Lists CloudFormation stacks."}
