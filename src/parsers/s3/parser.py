...
SERVICE = "s3"
SERVICE_NAME = "s3"
INTENTS = ["create_s3_bucket", "list_s3_buckets", "delete_s3_bucket", "list_s3_objects", "put_s3_object"]
...
def generate_command(intent, entities):
    if intent == "create_s3_bucket":
        ...
        return {"command": cmd, "explanation": f"Creates an S3 bucket named '{bucket}'{' in ' + region if region else ''}."}
    
    if intent == "list_s3_buckets":
        return {"command": "aws s3 ls", "explanation": "Lists S3 buckets in your account (global)."}
    
    if intent == "delete_s3_bucket":
        bucket = entities.get("bucket", "my-bucket")
        cmd = f"aws s3 rb s3://{bucket} --force"
        return {"command": cmd, "explanation": f"Deletes S3 bucket '{bucket}' and all its contents (force)."}
    ...
    if intent == "list_s3_objects":
        ...
        if prefix:
            ...
            return {"command": cmd, "explanation": f"Lists objects under '{prefix}' in bucket '{bucket}'."}
        else:
            cmd = f"aws s3 ls s3://{bucket}"
            return {"command": cmd, "explanation": f"Lists objects in bucket '{bucket}'."}
    ...
    if intent == "put_s3_object":
        ...
        return {"command": cmd, "explanation": f"Uploads local file '{local_path}' to s3://{bucket}/{key}."}
    
    return {"command": "echo 'Unknown S3 intent'", "explanation": "Intent not supported"}
