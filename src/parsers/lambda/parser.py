...
def generate_command(intent, entities):
    ...
    if intent == "list_lambda_functions":
        ...
        if region:
            cmd += f" --region {region}"
        return {"command": cmd, "explanation": f"Lists Lambda functions{' in ' + region if region else ''}."}
    if intent == "invoke_lambda_function":
        ...
        if region:
            cmd += f" --region {region}"
        return {"command": cmd, "explanation": f"Invokes Lambda function '{fn}' (result to /tmp/output.json)."}
