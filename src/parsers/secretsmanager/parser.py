...
def generate_command(intent, entities):
    ...
    if intent == "create_secret":
        ...
        # Never hardcode secret values - require user to provide via stdin or file
        cmd = f"aws secretsmanager create-secret --name {name} --secret-string file://secret.json"
        return {"command": cmd, "explanation": "Creates a new secret from file://secret.json (never hardcode secrets)."}

    return {"command": "echo 'Unsupported SecretsManager intent'", "explanation": "Unsupported intent."}
